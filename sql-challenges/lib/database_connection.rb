...
class DatabaseConnection
  ...
  def initialize(host, database_name)
    @host = host
  ...
  def run(sql, fields = [])
    @connection.exec_params(sql, fields)
  rescue PG::Error => e
    exit_with_helpful_query_message(e, sql, fields)
  end

  private

  def exit_with_helpful_connection_message(error)
  ...
  def exit_with_helpful_query_message(error, sql, fields)
